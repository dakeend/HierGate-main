...
def unbiased_curriculum_loss_self(out, label, args, criterion, scheduler='linear'):
    losses = []
    ...
    for idx in range(out.shape[0]):
        ground_truth = max(1, abs(label[idx].item()))
        loss = criterion(out[idx], label[idx])
        losses.append(loss)
    ...
    for i, loss in enumerate(losses):
        if adjusted_losses[i] > mean_loss + args.std_coff * std_loss:
            ...
            schedule_factor = scheduler(args.epoch, args.epochs) if args.anti_curri else 1 - scheduler(args.epoch,
                                                                                                       args.epochs)
