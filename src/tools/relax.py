...
def main():
    ...
    parser.add_argument('--rosetta-bin', type=str, dest="rosetta_bin", default='tools/rosetta/relax.static.linuxgccrelease',
                        help='Rosetta FastRelax binary executable.')
    parser.add_argument('-o', '--output-dir', type=str, dest="output_dir", default='data/pdbs/S1131',
    ...
    for l in open(args.mutant_list, 'r'):
        pdb_chain, pos, w, m ,ddg= l.strip().split()
    ...
    for pdb_chain, mutant in mutants:
        ...
        # #修改
        # low_pdb_name=pdb_chain.lower()
        # wild_type_struct = os.path.join(input_dir, low_pdb_name[:-1] + ".pdb")
